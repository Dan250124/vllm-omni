...
Examples:
    ...
    python bench.py --concurrency 10 --api-base http://192.168.1.100:8091
"""
...
class BenchStats:
    ...
    def report(self) -> None:
        print("\n" + "=" * 55)
        print("  Qwen3 TTS Benchmark Results")
        print("=" * 55)
...
def build_payload(text: str, voice: str, language: str) -> dict:
    return {
        "input": text,
        "voice": voice,
        "language": language,
        ...
        async def worker() -> None:
            ...
            while True:
                async with sem:
                    ...
                    payload = build_payload(text, voice, language)
                    r = await send_request(client, url, payload, rid, text)
                    stats.add(r)
