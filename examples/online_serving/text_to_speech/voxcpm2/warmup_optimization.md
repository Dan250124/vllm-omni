...
VoxCPM2 服务启动后，首次用户请求耗时 ~103s，第二次 ~10s，第三次及之后才稳定在 ~0.3s。

```
# timecost.txt 原始数据
19:08:36  torch.compile applied to: LocDiT, feat_encoder, AudioVAE, scaffold+residual (CUDA Graph)
19:10:13  CUDA Graph captured for scaffold (batch_size=1)   ← 首次请求耗时 ~103s
19:10:28  Response sent
```
...
| 阶段 | 耗时 | 说明 |
|------|------|------|
| `torch.compile` JIT | ~85s | LocDiT estimator、feat_encoder、AudioVAE、projections 的 Dynamo 追踪与编译 |
| `reduce-overhead` CUDA Graph | ~10s | torch.compile 内部 CUDA Graph 建立 |
| scaffold/residual CUDA Graph capture | ~5s | vLLM PagedAttention 的 CUDA Graph 捕获 |
| CJK 分词表构建 | ~5s | 19789 条目的 tokenizer 查表扫描 |
...
| 阶段 | 优化前 | 优化后 |
|------|--------|--------|
| `torch.compile` JIT | 首次用户请求 ~85s | 服务启动阶段 |
| `reduce-overhead` CUDA Graph | 首次用户请求 ~10s | 服务启动阶段 |
| 首次用户请求 CUDA Graph capture | ~5s | 服务启动阶段 |
| CJK 分词表构建 | ~5s | 服务启动阶段 |
| **首次用户请求总耗时** | **~103s** | **~1.9s** |
| **后续用户请求耗时** | ~0.3s | ~0.3s |
...
服务启动阶段增加约 **15s**（预热请求耗时），发生在 `Application startup complete` 日志之后、服务对外 ready 之前。对用户无感知。

### 补充说明

预热请求使用 `batch_size=1`，仅捕获该 batch 的 CUDA Graph。若后续出现 batch_size=2 的并发请求，首次会额外触发 ~1-2s 的 CUDA Graph 录制（无 torch.compile JIT 开销）。可通过配置 `max_num_seqs` 控制最大 batch。
